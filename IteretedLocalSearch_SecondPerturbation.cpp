...
std::vector<int> second_pert(const std::vector<int> &tour, lli &newCost, const lliMat &dstMtx){
    int N = tour.size();
    ...
    std::set<int> selected_indxs;
    while(selected_indxs.size() < 3) {
        int index = dis(gen);
        selected_indxs.insert(index);
    }

    std::vector<int> idxs(selected_indxs.begin(), selected_indxs.end());
    std::sort(idxs.begin(), idxs.end());

    // std::cout << "indices para dividir: ";
    // for(auto ele:idxs) std::cout << ele << ", ";
    // std::cout << std::endl; 

    perturbation.insert(perturbation.end(), tour.begin() + idxs[0] + 1, tour.begin() + idxs[1] + 1);
    perturbation.insert(perturbation.end(), tour.begin(), tour.begin() + idxs[0] + 1);
    perturbation.insert(perturbation.end(), tour.begin() + idxs[2] + 1, tour.end());
    perturbation.insert(perturbation.end(), tour.begin() + idxs[1] + 1, tour.begin() + idxs[2] + 1);

    newCost = computeCost(perturbation, dstMtx);
    return perturbation;
}
...
void iterative_loc_search(std::vector<int> &act_ans, lli &cost, const lliMat &dstMat, const mat &nearNodes,const float &alpha){
    ...
    while(CurrentExecutionTime(start) <= 180){
        ...
        perturbation = second_pert(act_ans, pertCost, dstMat); //Perturbacion
        ...
        if (pert_history.find(perturbation) == pert_history.end())
            ...
            perturbation = second_pert(perturbation, pertCost, dstMat); 
            pert_history.insert(perturbation);
        }
